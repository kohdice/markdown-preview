...
pub fn renderTableNode(
    ...
    for (0..col_count) |c| {
        if (c < table.header.len)
            col_widths[c] = @max(col_widths[c], try render_inline.renderedDisplayWidth(allocator, table.header[c], palette, link_defs, ambiguous));
        for (table.rows) |row| {
            if (c < row.len)
                col_widths[c] = @max(col_widths[c], try render_inline.renderedDisplayWidth(allocator, row[c], palette, link_defs, ambiguous));
        }
    ...
    try renderTableBorder(writer, col_widths, col_count, enable_ansi, palette, .top, ambiguous);
    try writer.writeByte('\n');

    try renderTableRow(allocator, writer, table.header, col_widths, table.alignments, col_count, enable_ansi, .{
        .fg = cell_fg,
        .bold = true,
    }, palette, link_defs, ambiguous);
    try writer.writeByte('\n');

    try renderTableBorder(writer, col_widths, col_count, enable_ansi, palette, .middle, ambiguous);
    ...
    try writer.writeByte('\n');
    try renderTableBorder(writer, col_widths, col_count, enable_ansi, palette, .bottom, ambiguous);
}
...
fn renderTableRow(
    ...
    for (0..col_count) |c| {
        ...
        const cell_width = try render_inline.renderedDisplayWidth(allocator, cell_text, palette, link_defs, ambiguous);
...
fn renderTableBorder(
    ...
    const glyph_w: usize = if (ambiguous == .wide) 2 else 1;
